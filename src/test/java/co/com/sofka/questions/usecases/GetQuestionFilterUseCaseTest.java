...
@SpringBootTest
class GetQuestionFilterUseCaseTest {
    ...
    void getByQuestionTest(){
        ...
        Mockito.when(questionRepository.findByQuestionLike(questionDTO.getCategory())).thenReturn(Flux.just(question));
        var resultQuestionDTO = getFilterUseCase.apply(questionDTO.getCategory()).collectList().block();
